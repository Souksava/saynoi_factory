<?php
   session_start();
    if($_SESSION['ses_id'] == ''){
      ...
      $id = $_GET['id'];
      $sqlget = "select billno,dateExp,timeExp,amount,cus_name,emp_name,x.status from tbexports x left join tbemployees e on x.emp_id=e.emp_id left join tbcustomers c on x.cus_id=c.cus_id where billno='$id';";
...
<head>
  ...
  <title>ລາຍລະອຽດຊ່ຳລະບິນຄ້າງຈ່າຍ</title>
...
<div class="wrapper">
  ...
  <div class="main-footer font14">
    ...
    <div class="card">
        <h5 class="card-header"><b>ລາຍລະອຽດຊ່ຳລະບິນຄ້າງຈ່າຍ</b></h5>
        <div class="card-body">
            <div style="width: 100%;">
                <div style="width: 48%;float: left;">
                    ...
                    <a href="#" class="btn btn-outline-success" data-toggle="modal" data-target="#exampleModalaccept">ຊ່ຳລະບິນຄ້າງຈ່າຍ</a>
                    <form action="creditdetail.php" method="POST" id="form1">
                        <div class="modal fade font14" id="exampleModalaccept" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body" align="center">
                                        ທ່ານຕ້ອງການຊ່ຳລະບິນຄ້າງຈ່າຍ ຫຼື ບໍ່ ?
                                        <input type="hidden" name="billno" value="<?php echo $rowget['billno'] ?>">
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-outline-secondary" data-dismiss="modal">ຍົກເລີກ</button>
                                        <button type="submit" name="btnPay" class="btn btn-outline-success">ຊ່ຳລະບິນຄ້າງຈ່າຍ</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div style="width: 48%;float: right;" align="right">
                    ...
                    ສະຖານະ: <?php echo $rowget['status'] ?> <br>
                </div>
            </div>
        </div>
      </div>
  ...
  <?php
    if(isset($_POST['btnPay'])){
        $billno = $_POST['billno'];
        $sqlPay = "update tbexports set status='ຈ່າຍແລ້ວ' where billno='$billno';";
        $resultPay = mysqli_query($link,$sqlPay);
        if(!$resultPay){
          echo"<script>";
          echo"window.location.href='credit.php?pay=found';";
          echo"</script>";
        }
        else{
          echo"<script>";
          echo"window.location.href='credit.php?pay2=success';";
          echo"</script>";
        }
    }
  ?>
