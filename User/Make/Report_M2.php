...
function ShowData(){
    ...
    if(isset($_POST['btnReport'])){
        $sql = "select pro_name,d.pro_id,d.qty,unit_name,img_path from tbmakedetail d left join tbproducts p on d.pro_id=p.pro_id left join tbunit u on p.unit_id = u.unit_id left join tbmaking m on d.make_no=m.make_no where d.make_no='$make_no' and m.status='ອະນຸມັດ' order by d.pro_id asc;";
        ...
        $sqlsum = "select sum(qty) as amount from tbmakedetail d left join tbmaking m on d.make_no=m.make_no where d.make_no='$make_no' and m.status='ອະນຸມັດ';";
