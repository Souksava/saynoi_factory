<?php
   session_start();
    if($_SESSION['ses_id'] == ''){
      ...
      $id = $_GET['id'];
      $sqlsec = "select pro_id,pro_name,qty,p.unit_id,unit_name,price,qtyalert,img_path from tbproducts p left join tbunit u on p.unit_id=u.unit_id where p.pro_id='$id'";
  ...
  <div class="main-footer font14">
    <form action="../../Check/Logout.php" method="POST" id="formLogout">
      <div class="modal fade font14" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          ...
          <form action="updatePro.php" id="form1" method="POST" enctype="multipart/form-data">
            <div class="row" align="left">
                <div class="col-xs-12 col-sm-6 form-control2">
                    <label>ຊື່ສິນຄ້າ</label>
                    <input type="text" name="pro_name" id="pro_name" placeholder="ຊື່ສິນຄ້າ" value="<?php echo $row['pro_name']; ?>">
                ...
                <div class="col-xs-12 col-sm-6 form-control2">
                    <label>ຈຳນວນ</label>
                    <input type="number" min="0" name="qty" id="qty" value="<?php echo $row['qty']; ?>" placeholder="ຈຳນວນ">
                    <i class="fas fa-check-circle "></i>
                    <i class="fas fa-exclamation-circle "></i>
                    <small class="">Error message</small>
                </div>
                <div class="col-xs-12 col-sm-6 form-control2">
                    <label>ລາຄາຂາຍ</label>
                    <input type="number" min="0" name="price" id="price" value="<?php echo $row['price']; ?>" placeholder="ລາຄາຂາຍ">
                    <i class="fas fa-check-circle "></i>
                    <i class="fas fa-exclamation-circle "></i>
                    <small class="">Error message</small>
                </div>
                <div class="col-xs-12 col-sm-6 form-control2">
                    ...
                    <?php
                        ...
                        $sqlunit = "select * from tbunit where unit_id !='$unit_id2';";
                ...
                <div class="col-xs-12 col-sm-6 form-control2">
                    ...
                    <input type="number" min="0" name="qtyalert" id="qtyalert" value="<?php echo $row['qtyalert'] ?>" placeholder="ເງື່ອນໄຂການຜະລິດ">
                    <i class="fas fa-check-circle "></i>
                    <i class="fas fa-exclamation-circle "></i>
                    <small class="">Error message</small>
                </div>
...
if(isset($_POST['pro_name'])){
  ...
  $img_path = $_FILES['img_path']['name'];
      if($img_path == ''){
        $sql = "update tbproducts set pro_name='$pro_name',qty='$qty',unit_id='$unit_id',price='$price',qtyalert='$qtyalert' where pro_id='$pro_id'";
        $result = mysqli_query($link, $sql);
        if(!$result)
          {
            echo"<script>";
            echo"window.location.href='product.php?update=false';";
            echo"</script>";
          }
        else{       
            echo"<script>";
            echo"window.location.href='product.php?update2=true';";
            echo"</script>";
        }
      }
      else{
          $sqlsec = "select  img_path from tbproducts where pro_id='$pro_id'";
          $resultsec = mysqli_query($link, $sqlsec);
          ...
          $sql = "update tbproducts set pro_name='$pro_name',qty='$qty',unit_id='$unit_id',price='$price',qtyalert='$qtyalert',img_path='$Pro_image' where pro_id='$pro_id'";
          $result = mysqli_query($link, $sql);
          if(!$result)
            {
              echo"<script>";
...
<script type="text/javascript">
  ...
  function checkInputs(){
    ...
    if(pro_idValue === ''){
      setErrorForUpdate(pro_id, 'ກະລຸນາປ້ອນລະຫັດສິນຄ້າ');
    }
    else{
      setSuccessForUpdate(pro_id);
    }
    if(pro_nameValue === ''){
      setErrorForUpdate(pro_name, 'ກະລຸນາປ້ອນຊື່ສິນຄ້າ');
    }
    else{
      setSuccessForUpdate(pro_name);
    }
    if(unit_idValue === ''){
      setErrorForUpdate(unit_id, 'ກະລຸນາເລືອກຫົວໜ່ວຍສິນຄ້າ');
    }
    else{
      setSuccessForUpdate(unit_id);
    }
    if(qtyValue === ''){
      setErrorForUpdate(qty, 'ປ້ອນຈຳນວນສິນຄ້າ');
    }
    else{
      setSuccessForUpdate(qty);
    }
    if(priceValue === ''){
      setErrorForUpdate(price, 'ກະລຸນາປ້ອນລາຄາສິນຄ້າ');
    }
    else{
      setSuccessForUpdate(price);
    }
    if(qtyalertValue === ''){
      setErrorForUpdate(qtyalert, 'ກະລຸນາເງື່ອນໄຂການສັ່ງຊື້');
    }
    else{
      setSuccessForUpdate(qtyalert);
    }
    if(pro_idValue !== '' && pro_nameValue !== '' && unit_idValue !=='' && qtyValue !== '' && priceValue !== '' && qtyalertValue !== ''){
      document.getElementById("form1").action = "updatePro.php";
